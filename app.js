...
function updateStreak(){
  ...
  if(diff===1){
    ...
    if(streakCount===7||streakCount===30||streakCount===100)trackEvent('streak_milestone','engagement',{days:streakCount});
...
function quickLog(){
  ...
  renderDailyLog();
  renderInsightsTab();
  trackEvent('daily_log','engagement',{amount:parseInt(amt),category:cat||'Other'});
...
function submitRating(){
  ...
  showRatingPrompt();
  renderSubReview();
  trackEvent('rate_subscription','engagement',{stars:stars});
...
function submitExpense(){const d={name:document.getElementById('fName').value.trim(),amount:parseInt(document.getElementById('fAmount').value),category:document.getElementById('fCategory').value.trim()||'Other',type:document.getElementById('fType').value,cycle:document.getElementById('fCycle').value,nextDue:document.getElementById('fNextDue').value,link:document.getElementById('fLink').value.trim(),upiId:document.getElementById('fUpiId').value.trim(),payType:document.getElementById('fPayType').value||'auto'};if(!d.name||!d.amount||!d.nextDue){showToast('Fill name, amount, due date');return;}addExpense(d);closeModal();loadData();
  trackEvent('add_expense','engagement',{expense_type:d.type,billing_cycle:d.cycle,pay_type:d.payType});
  showToast('Added');}
...
function trackEvent(action,category,params){if(typeof gtag==='function'){gtag('event',action,{event_category:category,...(params||{})});}}
function trackPage(view){if(typeof gtag==='function'){gtag('event','page_view',{page_title:view,page_location:location.href+'#'+view});}}
...
function renderInsights(){
  ...
  const monthData=months.map(m=>{
    let total=0;
    active.forEach(e=>{
      const created=new Date(e.createdAt||e.nextDue);
      const eKey=created.getFullYear()+'-'+created.getMonth();
      if(eKey<=m.key){
        let mo=e.amount;
        if(e.cycle==='quarterly')mo=e.amount/3;
        else if(e.cycle==='yearly')mo=e.amount/12;
        else if(e.cycle==='onetime')mo=0;
        total+=mo;
      }
    });
    total+=payments.filter(p=>{const d=new Date(p.date);return d.getFullYear()+'-'+d.getMonth()===m.key;}).reduce((s,p)=>s+p.amount,0);
    return Math.round(total);
  });
  destroyChart('trendChartInst');
  const trendCanvas=ensureCanvas('trendChart');
  const hasTrendData=active.length>0||payments.length>0;
  if(hasTrendData&&trendCanvas){
    trendChartInst=new Chart(trendCanvas,{type:'bar',data:{labels:months.map(m=>m.label),datasets:[{label:'Monthly Burn (₹)',data:monthData,backgroundColor:accent,borderRadius:6,barThickness:'flex',maxBarThickness:50}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,grid:{color:gridColor},ticks:{callback:v=>'₹'+fmtR(v)}},x:{grid:{display:false}}}}});
  }else if(trendCanvas){trendCanvas.parentElement.innerHTML='<div class=\"empty\"><div class=\"empty__text\">No expenses recorded yet</div></div>';}
  ...
  }else if(methodCanvas){methodCanvas.parentElement.innerHTML='<div class=\"empty\"><div class=\"empty__text\">No payments yet — mark expenses as paid to see breakdown</div></div>';}
}
